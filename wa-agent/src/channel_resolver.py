...
Duplicated verbatim from email-sync/src/channel_resolver.py — separate containers
can't share a Python module (same convention already used for
wa-agent/src/maintenance.py's _set_node_facts_wa vs. ingestor/src/graph.py's
set_node_facts). Keep the two copies in sync if either changes.
"""
import os
import psycopg2
