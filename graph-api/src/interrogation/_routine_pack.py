...
Duplicated verbatim from wa-agent/src/routine_context_pack.py for the
variance_pack interrogation primitive (Interrogation Layer, Increment 2) — a
copy, not a runtime HTTP call back into wa-agent, matching this project's own
established cross-container convention (same reasoning as wa-agent's
_set_node_facts_wa vs. ingestor's set_node_facts, and wa-agent's own
channel_resolver.py copy). wa-agent's two existing call sites
(main.py's live query path, maintenance.py's task_routine_context_pack) are
left untouched, still importing their local copy directly — moving/HTTP-ifying
it would make wa-agent's live query path newly dependent on graph-api's
uptime. Keep the two copies in sync if either changes.
