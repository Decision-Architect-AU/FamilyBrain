...
**GCal event tracking — two-layer identity** — every event written to Google Calendar carries a stable custom event ID (`fb{event_id:012x}`) for idempotent writes and a description tag (`[fb:eXXXXX]`) + `extendedProperties.private.fb_id` for ownership identification. When the stable ID is reserved in GCal's trash (409 conflict), the updater falls back to a fresh insert without a custom ID and stores the resulting Google-generated ID in `gcal_event_id`. The description tag survives this fallback and is used by `purge_gcal_duplicates.py` to detect and remove orphan duplicate events created by purge-and-restore cycles.
